    ...
    if (!id.equals("")) {
      ...
      if (!availableLinksLoaded) {
        if (linksXML.hasChildren()) {
          ...
          //XMLElement[] normalization = linksXML.getChildren("query/normalized/n");
          ...
          XMLElement querycontinue = linksXML.getChild("query-continue/links");
          if (querycontinue == null) {
            availableLinksLoaded = true;
            GenerativeDesign.unsort(availableLinks);
          } 
          else {
            String plcontinue = querycontinue.getStringAttribute("plcontinue");
      ...
      if (!availableBacklinksLoaded) {
        if (backlinksXML.hasChildren()) {
          // backlinksXML = backlinksXML.getChild(0);

          XMLElement[] children = backlinksXML.getChildren("query/backlinks/bl"); 
          for (int i = 1; i < children.length; i++) {
            String title = children[i].getStringAttribute("title");
            availableBacklinks.add(title);
          }

          XMLElement querycontinue = backlinksXML.getChild("query-continue/backlinks");
          if (querycontinue == null) {
            availableBacklinksLoaded = true;
            GenerativeDesign.unsort(availableBacklinks);
          } 
          else {
            String blcontinue = querycontinue.getStringAttribute("blcontinue");
