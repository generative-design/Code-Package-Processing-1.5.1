...
List sortedTags;
XMLElement[] pageXMLs = new XMLElement[0];
...
void setup() {
  ...
  bookXML = new XMLElement(this, "data/book.xml");
  int pageSum = 0;
  int tagSum = 0;
  XMLElement[] sections = bookXML.getChildren("sections/section");

  for (int i=0; i < sections.length; i++) {
    int pageCount = sections[i].getIntAttribute("pagecount"); 
    String pageName = sections[i].getStringAttribute("name");
    pageSum += pageCount;

    XMLElement[] tags = sections[i].getChildren();

    ArrayList tagList = new ArrayList();
    ArrayList inTagBlockPosition = new ArrayList();

    for (int j=0; j < tags.length; j++) {
      String tagName = tags[j].getStringAttribute("name");
    ...
    for (int k=0; k < pageCount; k++) {
      ...
      pageXMLs = (XMLElement[]) append(pageXMLs, sections[i]);
